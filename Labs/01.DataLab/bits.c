...
int addOK(int x, int y) {
    ...
    return !(sameSign&(xSign^sumSign));
}
/* 
 * conditional - same as x ? y : z //return y if x>0
...
int isAsciiDigit(int x) {
    int temp;
    x = ~x+1; //turns into a negative number
    x = x+0x39; //MUST BE >=0
    temp = ~x+1;
    temp = temp+9; //MUST BE >=0
    x = (x>>31)&1;
    temp = (temp>>31)&1;
...
int satAdd(int x, int y) {
    ...
    int sumSign = ((x+y)>>31)&1;
    int sameSign = !(xSign^ySign);
    int overflow =  !(sameSign&(xSign^sumSign)); //1 if there isn't any overflow
    ...
    return ((overflow)&(x+y)) | (~overflow)&((xSign)&neg_MIN|((~xSign)&pos_MAX));
}
/*
 * Extra credit
 */
